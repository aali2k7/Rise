...
export default function Navigation() {
  ...
  return (
    <nav
      className={`sticky top-0 z-50 bg-base-light/98 backdrop-blur-md border-b transition-smooth ${scrolled ? "border-base-subtle shadow-soft" : "border-transparent"
        }`}
    >
        ...
        <div className="flex justify-between items-center h-20">
          ...
          <div className="hidden lg:flex items-center space-x-10">
            {navLinks.map((link) => {
              ...
              // but for this flat structure exact match or just simple equality is fine.
              // We'll use strict equality for precise highlighting as requested.
              ...
              return (
                <Link
                  key={link.href}
                  href={link.href}
                  className={`text-sm font-medium transition-smooth relative group ${isActive ? "text-accent-navy" : "text-text-DEFAULT hover:text-accent-navy"
                    }`}
                >
                  {link.label}
                  <span
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  ...
                  className={`block px-4 py-3 text-sm font-medium rounded-md transition-smooth ${isActive ? "text-accent-navy bg-base" : "text-text-DEFAULT hover:bg-base"
                    }`}
                >
                  {link.label}
                </Link>
